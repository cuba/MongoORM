...
public extension MongoKitten.Collection {
    ...
    func insert<T: MongoEncodable>(_ object: T) throws -> EventLoopFuture<T> {
        var object = object
    ...
    /// - Throws: Either an error with mapping the object or a `MongoKitten` error.
    func insert<T: MongoEncodable>(_ objects: [T]) throws -> EventLoopFuture<[T]> {
        var objects = objects
    ...
    /// - Returns: An EventLoopFuture that returns the updated object.
    /// - Throws: Either an error with mapping the object or a `MongoKitten` error.
    func upsert<T: MongoEncodable>(_ object: T) throws -> EventLoopFuture<T> {
        var object = object
    ...
    /// - Returns: An EventLoopFuture that returns the updated object.
    /// - Throws: Either an error with mapping the object or a `MongoKitten` error.
    func update<T: MongoEncodable>(_ object: T) throws -> EventLoopFuture<T> {
        var object = object
    ...
    /// - Throws: Either an error with mapping the object or a `MongoKitten` error.
    func update<O: MongoObject>(object: O, valuesForKeys keys: [String]) throws -> EventLoopFuture<O> {
        var object = object
    ...
    func required(where query: Query) -> EventLoopFuture<Document> {
    ...
    /// - Returns: An EventLoopFuture that returns the document.
    func required(oid: ObjectId) -> EventLoopFuture<Document> {
    ...
    func required(oid: String) throws -> EventLoopFuture<Document> {
    ...
    func required<T: MongoDecodable>(where query: Query, type: T.Type) -> EventLoopFuture<T> {
    ...
    ///   - to: The type of object to decode.
    /// - Returns: The decoded object.
    func required<T: MongoDecodable>(oid: ObjectId, type: T.Type) -> EventLoopFuture<T> {
    ...
    ///   - to: The type of object to decode.
    /// - Returns: The decoded object.
    func required<T: MongoDecodable>(oid: String, type: T.Type) throws -> EventLoopFuture<T> {
    ...
    ///   - to: The type of object to decode.
    /// - Returns: The decoded object.
    func findOne<T: MongoDecodable>(where query: Query, type: T.Type) -> EventLoopFuture<T?> {
    ...
    ///   - to: The type of object to decode.
    /// - Returns: The decoded object.
    func findOne<T: MongoDecodable>(oid: ObjectId, type: T.Type) -> EventLoopFuture<T?> {
    ...
    ///   - to: The type of object to decode.
    /// - Returns: The decoded object.
    func findOne<T: MongoDecodable>(oid: String, type: T.Type) throws -> EventLoopFuture<T?> {
    ...
    func destroy(oid: ObjectId) -> EventLoopFuture<Int> {
    ...
    func destroy(oid: String) throws -> EventLoopFuture<Int> {
        let objectId = try ObjectId(oid)
    ...
    /// - Returns: Returns the EventLoopFuture that returns the number of objects deleted.
    func destroy<T: MongoObject>(_ object: T) -> EventLoopFuture<Int> {
    ...
    func exists(where query: Query) -> EventLoopFuture<Bool> {
    ...
    /// - Returns: Returns the EventLoopFuture that returns a boolean. This boolean is true if the object exists.
    func exists(oid: ObjectId) -> EventLoopFuture<Bool> {
    ...
    /// - Returns: Returns the EventLoopFuture that returns a boolean. This boolean is true if the object exists.
    /// - Throws: Throws if the hex string cannot be converted to an ObjectId.
    func exists(oid: String) throws -> EventLoopFuture<Bool> {
        let objectId = try ObjectId(oid)
    ...
    /// - Returns: Returns the EventLoopFuture that returns a boolean. This boolean is true if the object exists.
    func exists<T: MongoObject>(_ object: T) -> EventLoopFuture<Bool> {
